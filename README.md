...
Every record is sealed with an HMAC-SHA256 tag, bound to a monotonic version counter and an optional freshness window. If a single byte changes anywhere in the record either payload, header, or metadata, the store refuses to return it. If someone tries to replay an older valid record, the version check rejects it. If the entire on-disk container is modified (including its anti-replay state), the container MAC rejects it. If a key is rotated, old records remain verifiable under their original key ID.
...
| **Container MAC** | The on-disk container (counter, max_version, config, entry set) is authenticated end-to-end |
| **Metadata binding** | The record tag covers `key_id`, `version`, `timestamp`, and `nonce` — not just the payload |
| **Out-of-band keys** | Keys never touch the data file; `load()` accepts only caller-supplied keys |
...
| **Tombstones** | Deletion writes a signed tombstone, not a bare `del` |
| **Zero dependencies** | Pure Python 3.8+ standard library |
| **Tiny footprint** | < 400 lines of code, < 15 KB installed |
